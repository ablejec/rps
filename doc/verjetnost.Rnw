...
@
V R je funkcija parametrizirana na število neuspehov.
<<>>=
dnbinom(k-n,n,p)
...
(Y <- sum(X))
@
Zakaj $+1$ ? Poglejte v pomoè funkcije \code{rgeom}.
\end{frame}
%% <<<<<<---------------------|

%% |--------------------->>>>>>
\begin{frame}[fragile]
\frametitle{Potapljaški poskus}
\vspace{-0.5cm}
<<fig=TRUE,height=4>>=
...
t[2]/sum(t)
@
Z logiènimi vrednostmi lahko raèunamo:
<<>>=
sum(izid)
...
%% |--------------------->>>>>>
\begin{frame}[fragile]
\frametitle{Veliko število potapljaèev N}
<<>>=
N <- 100
n <- 350
p <- 0.2
...
%% |--------------------->>>>>>
\begin{frame}[fragile]
\frametitle{Število potopov za 80 biserov}
...
%% |--------------------->>>>>>
\begin{frame}[fragile]
\frametitle{Porazdelitev števila potopov}
<<fig=TRUE,height=4>>=
